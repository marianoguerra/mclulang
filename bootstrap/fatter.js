...
export function getType(v) {
  return v != null ? v[typeSym] : null;
}
export function setType(Cls, type) {
  Cls.prototype[typeSym] = type;
  return type;
}
export function mkType(name, Cls) {
  return setType(Cls, Symbol(name));
}
export class Frame {
  ...
  send(s, m) {
    ...
    if (handler) {
      ...
      throw new Error("HandlerNotFound", { cause: { s, m, e: this, handler } });
    }
  }
  bind(name, value) {
  ...
  logError = (err) => {
    switch (err.message) {
      ...
      case "HandlerNotFound": {
        const { s, m, handler } = err.cause;

        console.error(
          "Handler not found for type",
          s && getType(s),
          handler,
          `message '${m.verb}' value:`,
